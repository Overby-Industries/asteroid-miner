...
# itch.io app automatically). Mirrors helga-flight-sim's deploy_to_itch.ps1.
#
# Requires:
#   - A release GDExtension build already exists at
#     bin/libasteroidminer.windows.template_release.x86_64.dll (rebuild with
...
$ItchTarget = "aevoria-simulator/asteroid-miner:windows"
$BuildDir = Join-Path $PSScriptRoot "builds\windows-release"
$BuildExe = Join-Path $BuildDir "AsteroidMiner.exe"
...
Write-Host "Done. Live version: 1.0.$buildNumber -- check status with: $ButlerExe status aevoria-simulator/asteroid-miner"
